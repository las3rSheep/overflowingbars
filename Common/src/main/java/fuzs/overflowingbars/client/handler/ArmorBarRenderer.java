...
public class ArmorBarRenderer {
    ...
    public static void onRenderArmorBar(PoseStack poseStack, int posX, int posY, Player player, ProfilerFiller profiler) {
        ...
        boolean inverseColoring = OverflowingBars.CONFIG.get(ClientConfig.class).inverseColoring;
        boolean skip = OverflowingBars.CONFIG.get(ClientConfig.class).skipEmptyArmorPoints;
        int armorPoints = player.getArmorValue();
        int lastRowArmorPoints = OverflowingBars.CONFIG.get(ClientConfig.class).colorizeFirstRow || armorPoints / 20 > 0 ? armorPoints % 20 : 0;
        for (int currentArmorPoint = 0; currentArmorPoint < 10; ++currentArmorPoint) {
            if (armorPoints > 0) {
                ...
                if (currentArmorPoint * 2 + 1 < lastRowArmorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 18 : 36, 9, 9, 9, 256, 256);
                } else if (currentArmorPoint * 2 + 1 == lastRowArmorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 54 : 27, 9, 9, 9, 256, 256);
                } else if (currentArmorPoint * 2 + 1 < armorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 36 : 18, 9, 9, 9, 256, 256);
                } else if (currentArmorPoint * 2 + 1 == armorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 45 : 9, 9, 9, 9, 256, 256);
    ...
    public static void onRenderToughnessBar(PoseStack poseStack, int posX, int posY, Player player, ProfilerFiller profiler) {
        ...
        for (int currentArmorPoint = 0; currentArmorPoint < 10; ++currentArmorPoint) {
            if (armorPoints > 0) {
                ...
                if (currentArmorPoint * 2 + 1 < lastRowArmorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 18 : 36, 0, 9, 9, 256, 256);
                } else if (currentArmorPoint * 2 + 1 == lastRowArmorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 54 : 27, 0, 9, 9, 256, 256);
                } else if (currentArmorPoint * 2 + 1 < armorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 36 : 18, 0, 9, 9, 256, 256);
                } else if (currentArmorPoint * 2 + 1 == armorPoints) {
                    GuiComponent.blit(poseStack, startX, posY, inverseColoring ? 45 : 9, 0, 9, 9, 256, 256);
                } else if (!skip && currentArmorPoint * 2 + 1 > armorPoints) {

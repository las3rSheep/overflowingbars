...
public class GuiOverlayHandler {
    ...
    private static void handlePlayerHealthOverlay(RenderGameOverlayEvent.PreLayer evt) {
        if (evt.getOverlay() == ForgeIngameGui.PLAYER_HEALTH_ELEMENT && OverflowingBars.CONFIG.get(ClientConfig.class).health.allowLayers) {
            ...
            if (!minecraft.options.hideGui && gui.shouldDrawSurvivalElements()) {
                RenderSystem.enableBlend();
                BarOverlayRenderer.renderHealthLevelBars(evt.getMatrixStack(), evt.getWindow().getGuiScaledWidth(), evt.getWindow().getGuiScaledHeight(), minecraft, gui.left_height, OverflowingBars.CONFIG.get(ClientConfig.class).health.allowCount, OverflowingBars.CONFIG.get(ClientConfig.class).rowCount.showMaxHP);
                RenderSystem.disableBlend();

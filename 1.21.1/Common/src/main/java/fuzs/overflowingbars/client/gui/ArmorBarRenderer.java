    ...
    public static void renderArmorBar(GuiGraphics guiGraphics, int posX, int posY, int vOffset, int armorPoints, boolean left, boolean unmodified, ClientConfig.ArmorRowConfig config) {
        ...
        for (int currentArmorPoint = 0; currentArmorPoint < 10; ++currentArmorPoint) {
            ...
            if (currentArmorPoint * 2 + 1 < lastRowArmorPoints) {
                guiGraphics.blit(BarOverlayRenderer.OVERFLOWING_ICONS_LOCATION, startX, posY, inverse ? 18 : 36, vOffset, 9, 9, 256, 256);
            } else if (currentArmorPoint * 2 + 1 == lastRowArmorPoints) {
                if (armorPoints > 20) {
                    guiGraphics.blit(BarOverlayRenderer.OVERFLOWING_ICONS_LOCATION, startX, posY, inverse ? 54 : 27, vOffset, 9, 9, 256, 256);
                } else {
                    guiGraphics.blit(BarOverlayRenderer.OVERFLOWING_ICONS_LOCATION, startX, posY, inverse ? 9 : 45, vOffset, 9, 9, 256, 256);
                }
            } else if (currentArmorPoint * 2 + 1 < armorPoints) {
                guiGraphics.blit(BarOverlayRenderer.OVERFLOWING_ICONS_LOCATION, startX, posY, inverse ? 36 : 18, vOffset, 9, 9, 256, 256);
            } else if (currentArmorPoint * 2 + 1 == armorPoints) {
                guiGraphics.blit(BarOverlayRenderer.OVERFLOWING_ICONS_LOCATION, startX, posY, inverse ? 45 : 9, vOffset, 9, 9, 256, 256);
            } else if (!skip && currentArmorPoint * 2 + 1 > armorPoints) {
                guiGraphics.blit(BarOverlayRenderer.OVERFLOWING_ICONS_LOCATION, startX, posY, 0, vOffset, 9, 9, 256, 256);
            }
        }
    }
}
